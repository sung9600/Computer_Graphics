...
uint				NUM_TESS = 60;		// initial tessellation factor of the circle as a polygon
uint				Num_Circle = 100;
...
#endif
auto	circles = std::move(create_circles((float)Num_Circle));
...
{
	cur_Time = static_cast<float>(glfwGetTime());
	dt = cur_Time - t;
	t = float(glfwGetTime());
}
void update()
...
void render()
{
	auto checkOverlap = [](float x1, float y1, float r1, float x2, float y2, float r2) {
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS)
	{
		...
		else if (key == GLFW_KEY_EQUAL || key == GLFW_KEY_KP_ADD)
		{
			// CIRCLE NUM ++
			Num_Circle++;
			printf("\r- Num of Circles : %4d", Num_Circle);
		}
...
int main(int argc, char* argv[])
{
	...
	int prev_Num_Circle = Num_Circle;
	// enters rendering/event loop
	for (frame = 0; !glfwWindowShouldClose(window); frame++)
	{
		if (prev_Num_Circle != Num_Circle)
		{
			prev_Num_Circle = Num_Circle;
			circles = create_circles((float)Num_Circle);
		}
